...
var SELECTED_TYPES = ["vmess", "ss", "ssr", "trojan", "snell", "http", "socks5"];
...
function mixConfig(cnf, url, ip, operator, provider) {
  try {
    var conf = { ...cnf };
...
function validateConfig(cnf, provider) {
  try {
    var conf = { ...cnf };
...
function toClash(configList) {
  clash.proxies = configList.map((conf) => (({ provider, merged, ...others }) => others)(conf));
  ...
  for (const worker in groupedConfigs) {
    groupedConfigs[worker] = groupedConfigs[worker].reduce((group, conf) => {
  ...
  async fetch(request) {
    ...
    if (type2 === "clash") {
      ...
      if (includeOriginalConfigs && url.searchParams.has("merge")) {
        const merge2 = url.searchParams.get("merge");
        onlyOriginalConfigs = !["1", "true", "yes", "y"].includes(merge2.toLowerCase());
      }
      ...
      var finalConfigList = [];
      var newConfigs;
      const configPerList = Math.floor(maxConfigs / Object.keys(configProviders).length);
      for (const provider in configProviders) {
        try {
          if (selectedProviders.length > 0 && !selectedProviders.includes(provider)) {
            continue;
          }
          const content = await fetch(configProviders[provider]).then((r) => r.text());
          ...
          if (includeOriginalConfigs) {
            configList.push({
              name: provider,
              count: configPerList,
              configs: newConfigs.filter((cnf) => selectedTypes.includes(cnf.type)),
              renamedConfigs: null
            });
          }
        } catch (e) {
      ...
      for (const operator of operators) {
        ...
        for (const i2 in acceptableConfigList) {
          ...
          acceptableConfigList[i2].mergedConfigs = el.configs.map((cnf) => mixConfig(cnf, url, ip, operator, el.name)).filter((cnf) => cnf.merged && cnf.name);
        }
        var remaining = 0;
        for (var i = 0; i < 5; i++) {
      ...
      if (includeOriginalConfigs) {
        for (const i2 in configList) {
          ...
          configList[i2].renamedConfigs = el.configs.map((cnf) => validateConfig(cnf, el.name)).filter((cnf) => cnf.name);
        }
        var remaining = 0;
        for (var i = 0; i < 5; i++) {
